...
list_t* CreateList(int (*compare)(void*, void*), void (*print)(void*, void*, char*),
                   void (*delete)(void*)) {
...
void InsertAtHead(list_t* list, void* val_ref) {
	...
	printf("ERROR: List or insertion item is NULL.\n");
        return;
    }
...
void InsertInOrder(list_t* list, void* val_ref) {
    ...
    int comp_val = list->comparator(new_node->data, (*head)->data);
    if (comp_val == 0xBEEFCAFE) {
	    printf("ERROR: No insertion. Comparator returned error.\n");
	    return ;
    }
    ...
    if (comp_val == -1) {
        ...
        *head = new_node;
        ...
        while (current != NULL) {
	    comp_val = list->comparator(new_node->data, current->data);
            if (comp_val == 1) {
...
void PrintLinkedList(list_t* list, FILE* fp, char* sep) {
    if(list == NULL)
        return;
